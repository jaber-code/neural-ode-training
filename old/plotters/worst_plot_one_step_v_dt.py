"""
Top-50 worst cases when the model (trained at dt=0.2) is run at a DIFFERENT dt.

Change TEST_DT to probe how the field generalizes across time steps.
Prediction and ground truth BOTH use TEST_DT (consistent comparison).

  black dot  = start
  green sq   = true next position at TEST_DT
  red x      = predicted next position at TEST_DT
  orange     = action direction
"""
import numpy as np
import torch
...
TEST_DT = 0.1         # <-- the dt to test at (train was 0.2)
N_WORST = 50
...
with torch.no_grad():
    ...
    pred = S1 + V * TEST_DT                              # prediction at TEST_DT
    true = torch.clamp(S1 + A * TEST_DT, 0.0, 1.0)       # truth at SAME dt
    err = (pred - true).norm(dim=1) // 0.2/8
